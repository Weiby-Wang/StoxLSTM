...
do
    python -u main.py \
        ...
        --weight_decay 1e-3 \
        --device 'cuda' \
        --figure_save_path ./figure/$data/$data'_'$prediction_length'_wo_decomp'.png \
        --pre_train_wts_load_path  "" \
        --wts_load_path ./_wts/$data/$data'_'$prediction_length'_wo_decomp'.ckpt \
        --wts_save_path ./_wts/$data/$data'_'$prediction_length'_wo_decomp'.ckpt > ./train_info/$data/$data'_'$prediction_length'_wo_decomp'.txt 2>&1
done