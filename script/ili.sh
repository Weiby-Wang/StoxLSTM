...
for prediction_length in 24 36 48
do
    python -u main.py \
        ...
        --wts_save_path ./_wts/$data/$data'_'$prediction_length.ckpt > ./train_info/$data/$data'_'$prediction_length.log 2>&1
done
...
python -u main.py \
    ...
    --weight_decay 1e-5 \
    --device 'cuda' \
    --figure_save_path ./figure/$data/$data'_60'.pdf \
    --pre_train_wts_load_path  "" \
    --wts_load_path ./_wts/$data/$data'_60'.ckpt \
    --wts_save_path ./_wts/$data/$data'_60'.ckpt > ./train_info/$data/$data'_60'.log 2>&1
